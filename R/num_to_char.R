...
num_to_char <- function(x, digits = NA, na_as_string = NA, use_signif = FALSE, always_decimal = FALSE, na_as_null = FALSE) {
  if (is.na(digits)) digits <- NA_integer_
  stopifnot(is.numeric(x))
  stopifnot(is.numeric(digits))
  stopifnot(is.logical(na_as_string))
  stopifnot(is.logical(na_as_null))
  .Call(R_num_to_char, x, digits, na_as_string, use_signif, always_decimal, na_as_null)
}
